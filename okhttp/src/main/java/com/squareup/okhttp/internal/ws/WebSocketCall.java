...
public class WebSocketCall {
   ...
   * necessarily indicate application-layer success: {@code response} may still indicate an unhappy
   * HTTP response code like 404 or 500.
   *
   * @throws IOException if the request could not be executed due to a connectivity problem or
   *     timeout. Because networks can fail during an exchange, it is possible that the remote
   *     server accepted the request before the failure.
   *
   * @throws IllegalStateException when the web socket has already been connected.
   */
  public WebSocketResponse execute(WebSocketListener listener) throws IOException {
    ...
    WebSocket webSocket = websocketize(connection, listener);

    return new WebSocketResponse(response, webSocket);
  }
   ...
   */
  public void enqueue(final WebSocketCallback responseCallback, final WebSocketListener listener) {
    call.enqueue(new Callback() {
      ...
      @Override public void onResponse(Response response) throws IOException {
        // TODO try/catch? What to do with ProtocolException?
        validateResponse(response);
        ...
        WebSocket webSocket = websocketize(connection, listener);
        responseCallback.onResponse(response, webSocket);
      }
    });
  }
  ...
  private WebSocket websocketize(Connection connection, WebSocketListener listener)
      throws IOException {
